...
calls, upstream callback verification, and internal HMAC forwarding.
...
- Added `PAYMENT_PROVIDER=official`, `OFFICIAL_PAY_GATEWAY_URL`,
  `OFFICIAL_PAY_GATEWAY_SECRET`, and `PAYMENT_RECONCILE_SECRET`.
...
- Added HMAC-SHA256 signing between Next.js and PHP gateway with a 5 minute
  timestamp window.
